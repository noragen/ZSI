...
class ParsedSoap:
        ...
        for n in _children(elt):
            ...
            if mustqualify and not n.namespaceURI:
                raise ParseException('Unqualified element "'
                        + n.nodeName + '" in ' + name, inheader, elt,
                        self.dom)
    ...
    def FindLocalHREF(
        ...
        if href[0] != '#':
            raise EvaluateException('Absolute HREF ("%s") not implemented'
    ...
    def GetMyHeaderElements(self, actorlist=None):
        ...
        return [E for E in self.header_elements if _find_actor(E)
                in actorlist]
    ...
    def WhatMustIUnderstand(self):
        ...
        return [(E.namespaceURI, E.localName) for E in
                self.header_elements if _find_mu(E) == '1']
