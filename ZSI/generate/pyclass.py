...
class pyclass_type(type):
    ...
    def __new__(
        ...
        # Assume this means mutable type. ie. ofwhat.
            ...
            assert hasattr(typecode, 'ofwhat'), \
                'typecode has no ofwhat list??'
            ...
            if typecode.mixed:
                (get, set) = \
                    cls.__create_text_functions_from_what(typecode)

                if get.__name__ in classdict:
                    raise AttributeError('attribute %s previously defined.' \
                        % get.__name__)
            ...
            for what in typecode.ofwhat:
                (get, set, new_func) = \
                    cls.__create_functions_from_what(what)
                ...
                if pname is None and isinstance(what, TC.AnyElement):
                    pname = 'any'
                assert pname is not None, 'Element with no name: %s' \
                    % what
                ...
                if __debug__:
                    assert pname not in pydoc.Helper.keywords, \
                ...
                classdict[pname] = property(get, set, None,
                        'property for element (%s,%s), minOccurs="%s" maxOccurs="%s" nillable="%s"'
                         % (what.nspname, what.pname, what.minOccurs,
                        what.maxOccurs, what.nillable))
        ...
        if hasattr(typecode, 'attribute_typecode_dict'):
            attribute_typecode_dict = typecode.attribute_typecode_dict \
                or {}
            for (key, what) in list(attribute_typecode_dict.items()):
                (get, set) = cls.__create_attr_functions_from_what(key,
                        what)
                if get.__name__ in classdict:
                    raise AttributeError('attribute %s previously defined.' \
                        % get.__name__)
    ...
    def __create_functions_from_what(what):
        ...
        if not callable(what) and getattr(what, 'pyclass', None) \
            is None:
            new_func = None
        elif isinstance(what, TC.ComplexType) or isinstance(what,
                TC.Array):
        ...
        elif issubclass(what.klass, TC.ComplexType) \
            or issubclass(what.klass, TC.Array):
    ...
    __create_functions_from_what = \
        staticmethod(__create_functions_from_what)
